...
#SBATCH --job-name=200cells           # Assign an short unique name to your job
#SBATCH --requeue
#SBATCH --nodes=1                   # Number of nodes you require
#SBATCH --ntasks=1                  # Total # of tasks across all nodes
#SBATCH --cpus-per-task=1           # Cores per task (>1 if multithread tasks)
#SBATCH --mem=25600                  # Real memory (RAM) required (MB)
#SBATCH --time=02:00:00             # Total run time limit (HH:MM:SS) // Days: 1-3:0:0
###SBATCH --array=1-10
#SBATCH --output=/dev/null #%A_%a.out          # STDOUT output file
#SBATCH --error=./out-slurm/%A_%a.err           # STDERR output file (optional)
