...
#dyn.load('/home/XXXX/miniconda3/lib/libglpk.so.40');
library(Seurat)
library(magrittr)
library(corrplot)
...
p.w_list = lapply(paths, List_p.w)

names(p.w_list) = gsub("/home/XXXX/denSNE/9.0stat_batches/out.MWW/20220309-var50_1.5_3.0-6type-", "", paths)
names(p.w_list) = gsub("\\/", "", names(p.w_list))

saveRDS(p.w_list, 
        sprintf("/home/XXXX/denSNE/9.0stat_batches/summarize-p.vals/%s-list-p.w-MWW.Rds", datetag))
