...
files2 = list.files(path.in, pattern = "Median.Var.byAbundance.txt", recursive = T)
#files1number = files1 %>% gsub("/Median.Var.txt",  "", .) %>% as.numeric()
files2number = files2 %>% gsub("/Median.Var.byAbundance.txt",  "", .) %>% as.numeric()
...
mclapply(not.avail, mc.cores = 7,
         function(X){
           ...
           var_melt.merge_by.genegroup = 
             merge(var_melt.merge, gene.mean.altered %>% ungroup %>% select(Gene.CellType, group), 
                   by = "Gene.CellType", 
                   all.x = TRUE)
           ### remove NA rows
           ...
           var_melt.merge_by.genegroup$logVar = log10(var_melt.merge_by.genegroup$var + 1)
           
           ### add celltype gene
           var_melt.merge_by.genegroup$Gene.CellType = paste0(var_melt.merge_by.genegroup$Gene, ".", var_melt.merge_by.genegroup$CellType)
           
           ## subset only altered genes
           var_melt.merge_by.genegroup = var_melt.merge_by.genegroup[var_melt.merge_by.genegroup$Gene.CellType %in% gene.celltype_altered,  ]
           
           ## get var median --------------------------------------------
           var.median_counts.melt.altered  = 
             setDT(var_melt.merge_by.genegroup)[, .(var.median=median(var)), 
                                    .(CellType, condition, NormMethod, group)]
           ## dcast var mean 
           var.median_counts.melt.altered = dcast(var.median_counts.melt.altered, 
                                                  CellType + NormMethod + group~ condition, 
           ...
           write.table(as.data.frame(var.median_counts.melt.altered), 
                       #"%s/MedianVar.txt", 
                       sprintf("%s/Median.Var.byAbundance.txt", outpath), 
