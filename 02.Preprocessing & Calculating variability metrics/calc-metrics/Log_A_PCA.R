...
path.in = sprintf("/home/XXX/denSNE/8.0calculate-affinity_batches/out/%s-var%.0f_%.1f_%.1f-6type-%scell", 
                  ...
                  factor_upper, 
                  ncells)
files = list.files(path.in, pattern = "method3.txt", recursive = T)
filesnumber = files %>% gsub("/method3.txt",  "", .) %>% as.numeric()
not.avail = c(minfolder:maxfolder) %>% .[!. %in% filesnumber]
print(sprintf("not avail in %s", path.in))
print(not.avail)

mclapply(not.avail, mc.cores = 3,
         function(X){
           # source functions -------------------------------------------------
