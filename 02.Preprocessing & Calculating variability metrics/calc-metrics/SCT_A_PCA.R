...
path.in = sprintf("/home/XXX/denSNE/8.0calculate-affinity_batches/out/%s-var%.0f_%.1f_%.1f-6type-%scell", 
                  ...
                  factor_upper, 
                  ncells)
files = list.files(path.in, pattern = "method12.txt", recursive = T)
filesnumber = files %>% gsub("/method12.txt",  "", .) %>% as.numeric()
not.avail = c(minfolder:maxfolder) %>% .[!. %in% filesnumber]
print(sprintf("not avail in %s", path.in))
print(not.avail)

mclapply(not.avail, mc.cores = 6,
         function(X){
           # source functions -------------------------------------------------
           ...
           # print("Normalizing")
           obj = SCTransform(obj, return.only.var.genes = F, verbose = F)
           
           # PCA ---------------------------------------------------------------------
           # print("Performing PCA")
           obj = RunPCA(obj, assay = "SCT", features = rownames(obj), 
