.libPaths(c(.libPaths(), "/home/XXX/R/rocker-rstudio/4.0/"))
...
path.in = sprintf("/home/XXX/denSNE/8.0calculate-affinity_batches/out/%s-var%.0f_%.1f_%.1f-6type-%scell", 
                  ...
                  factor_upper, 
                  ncells)
files = list.files(path.in, pattern = "method38.txt", recursive = T)
filesnumber = files %>% gsub("/method38.txt",  "", .) %>% as.numeric()
