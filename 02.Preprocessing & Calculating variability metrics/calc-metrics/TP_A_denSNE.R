...
path.in = sprintf("/home/XXX/denSNE/8.0calculate-affinity_batches/out/%s-var%.0f_%.1f_%.1f-6type-%scell", 
                  ...
                  factor_upper, 
                  ncells)
files = list.files(path.in, pattern = "method37.txt", recursive = T)
filesnumber = files %>% gsub("/method37.txt",  "", .) %>% as.numeric()
...
lapply(not.avail, #mc.cores = 5, 
         function(X){
           ...
           write.table(affinity, 
                       sprintf("%s/method37.txt", outpath), 
