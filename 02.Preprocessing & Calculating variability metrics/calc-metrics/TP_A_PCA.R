...
path.in = sprintf("/home/XXX/denSNE/8.0calculate-affinity_batches/out/%s-var%.0f_%.1f_%.1f-6type-%scell", 
                  ...
                  factor_upper, 
                  ncells)
files = list.files(path.in, pattern = "method1.txt", recursive = T)
filesnumber = files %>% gsub("/method1.txt",  "", .) %>% as.numeric()
